...
Cephadm handles the Prometheus, Grafana, and Alertmanager configurations
automatically.

However, there is one exception to this rule. In a somewhat difficult setup, the
users browser might not be able to access the URL to Grafana configured in Ceph
Dashboard. The cluster being in a different DNS zone than the accessing user
might be such a scenario.

For this case, there is an extra configuration option for Ceph Dashboard, which
can be used to configure which URL for accessing Grafana by the users browser
should be used. This value will never be altered by cephadm. To set this
